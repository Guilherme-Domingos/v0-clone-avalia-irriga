...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      await authClient.signIn.social({
        provider: 'google',
        callbackURL: 'http://localhost:3000/perfil',
      });
    } catch (err: any) {
  ...
  useEffect(() => {
    ...
    // Se não tem token, depende da sessão
    if (isPending) return;
    if (session) {
      // Salva token para futuras verificações e redireciona
      if (typeof window !== 'undefined') {
        localStorage.setItem('auth-token', 'authenticated');
      }
      router.replace('/perfil');
        ...
        <Card className='p-6 md:p-8 shadow-xl'>
          <div className='flex flex-col items-center justify-center mb-8'>
            ...
            <p className='text-gray-600 text-center mt-2'>
              Sistema de avaliação de irrigação localizada
            </p>
          </div>
