...
export default function NovaAvaliacaoPivoCentral({
  params,
}: {
    ...
    input.onchange = (e) => {
      ...
      if (file) {
        ...
        for (const dado of dadosSimulados) {
          const index = novosVolumes.findIndex(
            (v) => v.raio === dado.raio && v.distancia === dado.distancia
          );
          if (index !== -1) {
  ...
  const validarDados = (): boolean => {
    ...
    for (const raio of raios) {
      const volumesRaio = volumes.filter(
        (v) => v.raio === raio && v.volume !== null && v.volume > 0
      );
      const percentualPreenchido =
        (volumesRaio.length / totalPontosPorRaio) * 100;
        ...
        <main className='flex-1 p-4 space-y-6 max-w-5xl mx-auto w-full'>
            ...
            <div className='space-y-4'>
              <div className='space-y-2'>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleDadosChange('nomeAvaliacao', e.target.value)
                  }
                  required
                />
              </div>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                <div className='space-y-2'>
                  <Label htmlFor='diametroColetor'>
                    Diâmetro do Coletor (cm)
                  </Label>
                  <Input
                    id='diametroColetor'
                    type='number'
              ...
              <div className='p-3 bg-gray-100 rounded-md'>
                <p className='text-sm font-medium text-gray-700'>
                  Área do Coletor: {areaColetor.toFixed(2)} cm²
                </p>
              </div>
            </div>
          </Card>
          ...
          <div className='flex flex-col sm:flex-row gap-4 justify-center'>
            ...
            <Button
              variant='ghost'
              onClick={() => router.back()}
              className='text-red-600 hover:bg-red-50 px-6 py-3'>
              Cancelar/Voltar
            </Button>
          </div>
        </main>
