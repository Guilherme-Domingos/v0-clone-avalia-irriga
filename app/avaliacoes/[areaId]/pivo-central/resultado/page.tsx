...
export default function ResultadoAvaliacaoPivo({
  params,
}: {
  ...
  const calcularMetricas = (dados: any): ResultadoMetricas => {
    ...
    const mediaGeralLamina =
      dadosDetalhados.reduce((sum, d) => sum + d.lamina, 0) /
      dadosDetalhados.length;

    // Calcular diferença percentual para a média
    dadosDetalhados.forEach((d) => {
      d.diferencaMedia =
        ((d.lamina - mediaGeralLamina) / mediaGeralLamina) * 100;
    });

    // Calcular métricas por raio
    const raios = Array.from(
      { length: dadosPivo.numeroRaios },
      (_, i) => `R${i + 1}`
    );
    const metricasPorRaio = raios.map((raio) => {
      const laminasRaio = dadosDetalhados
        .filter((d) => d.raio === raio)
        .map((d) => d.lamina);

      const mediaLamina =
        laminasRaio.reduce((sum, l) => sum + l, 0) / laminasRaio.length;
  ...
  const getInterpretacaoAutomatica = (): string => {
    ...
    if (cucGeral < 90) {
      interpretacao += `A uniformidade (CUC) geral de ${cucGeral.toFixed(
        2
      )}% está abaixo do recomendável (90%), indicando necessidade de ajuste nos aspersores. `;
    } else {
      interpretacao += `A uniformidade (CUC) geral de ${cucGeral.toFixed(
        2
      )}% está dentro do padrão recomendável. `;
    }

    if (cudGeral < 70) {
      interpretacao += `O CUD geral de ${cudGeral.toFixed(
        2
      )}% sugere que 25% dos pontos coletados apresentaram lâmina muito baixa. `;
    }

    const raioProblematico = metricasPorRaio.find((r) => r.cuc < 85);
    if (raioProblematico) {
      interpretacao += `Especial atenção ao ${
        raioProblematico.raio
      }, onde a média de lâmina foi ${raioProblematico.mediaLamina.toFixed(
        2
      )}mm e o CUC = ${raioProblematico.cuc.toFixed(2)}%. `;
    }
      ...
      <div className='flex flex-col min-h-screen bg-gray-50'>
        ...
        <main className='flex-1 p-4 space-y-6 max-w-5xl mx-auto w-full'>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4'>
            ...
            <Card className='p-4 shadow-md rounded-2xl'>
              <h3 className='font-bold text-center mb-2'>
                Média Geral de Lâmina
              </h3>
              <p className='text-center text-lg'>
                {resultados.mediaGeralLamina.toFixed(2)} mm
              </p>
            </Card>
          ...
          <Card className='p-4 shadow-md'>
            <h2 className='text-xl font-bold mb-4'>
              Distribuição da Lâmina por Raio
            </h2>
              ...
              {resultados.metricasPorRaio.map((metrica) => (
                <Card
                  ...
                  <div className='space-y-2 text-center'>
                    <p className='text-sm'>
                      <span className='font-medium'>Média de Lâmina:</span>{' '}
                      {metrica.mediaLamina.toFixed(2)} mm
                    </p>
                    <p className='text-sm'>
                      <span className='font-medium'>CUC ({metrica.raio}):</span>{' '}
                      {metrica.cuc.toFixed(2)}%
                    </p>
                    <p className='text-sm'>
                      <span className='font-medium'>CUD ({metrica.raio}):</span>{' '}
                      {metrica.cud.toFixed(2)}%
                    </p>
                  </div>
                </Card>
              ))}
          ...
          <div className='flex flex-col sm:flex-row gap-4 justify-center'>
            <Button
              variant='outline'
              className='border-gray-400 text-gray-700 hover:bg-gray-100 flex items-center gap-2'>
