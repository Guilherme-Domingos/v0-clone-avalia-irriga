...
import { ChevronLeft, Download, BarChart, Calendar, Lightbulb, MessageSquare } from "lucide-react"
...
export default function ResultadoAvaliacaoPivo({ params }: { params: { areaId: string } }) {
  ...
  const simularResultados = () => {
    ...
    setResultados({
      ...
      comentarios: [
        {
          id: "1",
          texto: "Pressão baixa observada no setor R3, necessário verificar bomba",
  ...
  const calcularMetricas = (dados: any): ResultadoMetricas => {
    const { dadosPivo, volumes, comentarios, areaColetor, dataAvaliacao } = dados
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-4 space-y-6 max-w-5xl mx-auto w-full pb-20">
        {/* Grid de Resumo */}
        ...
        {resultados.comentarios.length > 0 && (
          <Card className="p-4 shadow-md">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-bold">Comentários e Observações</h2>
            </div>
            <div className="space-y-3">
              {resultados.comentarios.map((comentario) => (
                <Card key={comentario.id} className="p-3 bg-gray-50 border border-gray-200">
                  <p className="text-sm mb-2">{comentario.texto}</p>
                  {comentario.imagem && (
                    <div className="mt-2">
                      <Image
